...
matr_pca = np.array(pd.DataFrame(pd.read_csv('../Data/redc/U_ROMS_RPCA100.txt', header=None)), dtype=np.float32)
...
m_reduced = 100  # feature space dimension
c_reduced = [17, 34, 51]
c_idx = 0
# algorithm parameter
n = 20  # number of nest
...
def write_cnop():
    ...
    mat1 = tf.placeholder(tf.float32, [None, c_reduced[c_idx]])
    mat2 = tf.placeholder(tf.float32, [c_reduced[c_idx], m_origin])
    ret_mat = tf.matmul(mat1, mat2)
    best_origin = []
    with tf.Session(config=cpu_config) as sess:
        sess.run(tf.global_variables_initializer())
        generator_out = sess.run(ret_mat,
                                 feed_dict={mat1: best[:c_reduced[c_idx]].reshape(-1, c_reduced[c_idx]),
                                            mat2: matr_pca[:, :c_reduced[c_idx]].T})
...
def cal_constraint_sol(solution):
    ...
    with tf.Session(config=cpu_config) as sess:
        sess.run(tf.global_variables_initializer())
        generator_out = sess.run(ret_mat,
                                 feed_dict={mat1: solution.reshape(-1, c_reduced[c_idx]),
                                            mat2: matr_pca[:, :c_reduced[c_idx]].T})
...
def initial_solutions():
    ...
    mat2 = tf.placeholder(tf.float32, [m_origin, c_reduced[c_idx]])
    ret_mat = tf.matmul(mat1, mat2)
    encoder_solution = []
    with tf.Session(config=cpu_config) as sess:
        sess.run(tf.global_variables_initializer())
        generator_out = sess.run(ret_mat,
                                 feed_dict={mat1: origin.reshape(-1, m_origin), mat2: matr_pca[:, :c_reduced[c_idx]]})
        encoder_solution = generator_out.reshape(c_reduced[c_idx])
    nest[0, :c_reduced[c_idx]] = encoder_solution
    vel[0, :] = 0
    ...
    for i in range(1, n):
        for j in range(0, m_reduced):
            ...
            if j < c_reduced[c_idx]:
                nest[i, j] = encoder_solution[j] + vel[i, j]

    # project the initial swarm
    for i in range(n):
        nest[i, :c_reduced[c_idx]] = project(nest[i, :c_reduced[c_idx]], i)
...
def getNewNest(weight):
    ...
    for i in range(n):
        ...
        nest[i, :c_reduced[c_idx]] = nest[i, :c_reduced[c_idx]] + vel[i, :c_reduced[c_idx]]
        nest[i, :c_reduced[c_idx]] = project(nest[i, :c_reduced[c_idx]], i)
...
def getBestNest():
    ...
    for i in range(n):
        ...
        fnew = adapFunction(nest[i, :c_reduced[c_idx]])
        if fnew > localfit[i]:
            localfit[i] = fnew
    ...
    sum = cal_constraint_sol(best[:c_reduced[c_idx]])
    print('constraint of current best: %e' % sum)

    return

# shift the solution between different space
def resizeSolution():
    global n, nest, localnest, best, localfit, fbest

    print('----resize solution from', c_reduced[c_idx - 1], 'to', c_reduced[c_idx], '----')
    for i in range(n):
        print(i)
        # decode the low dimension solution
        mat1 = tf.placeholder(tf.float32, [None, c_reduced[c_idx - 1]])
        mat2 = tf.placeholder(tf.float32, [c_reduced[c_idx - 1], m_origin])
        ret_mat = tf.matmul(mat1, mat2)
        solution_origin = []
        with tf.Session(config=cpu_config) as sess:
            sess.run(tf.global_variables_initializer())
            generator_out = sess.run(ret_mat,
                                 feed_dict={mat1: localnest[i, :c_reduced[c_idx - 1]].reshape(-1, c_reduced[c_idx - 1]),
                                            mat2: matr_pca[:, :c_reduced[c_idx - 1]].T})
            solution_origin = generator_out.reshape(m_origin)
        # encode the high dimension solution
        mat1 = tf.placeholder(tf.float32, [None, m_origin])
        mat2 = tf.placeholder(tf.float32, [m_origin, c_reduced[c_idx]])
        ret_mat = tf.matmul(mat1, mat2)
        encoder_solution = []
        with tf.Session(config=cpu_config) as sess:
            sess.run(tf.global_variables_initializer())
            generator_out = sess.run(ret_mat,
                                     feed_dict={mat1: solution_origin.reshape(-1, m_origin),
                                                mat2: matr_pca[:, :c_reduced[c_idx]]})
            encoder_solution = generator_out.reshape(c_reduced[c_idx])
        nest[i, :c_reduced[c_idx]] = encoder_solution
        nest[i, c_reduced[c_idx]:] = 0
...
write_cnop()
print('------CNOP written in ../data/cnop.nc------', c_reduced[c_idx])
